...
const modalIndex = content.indexOf('{/* Connect Modal');
if (modalIndex === -1) {
    console.error("Could not find Connect Modal section");
    process.exit(1);
}
            ...
            <div className={clsx("p-6 md:p-8 overflow-y-auto custom-scrollbar", connectStep === 1 ? "max-h-[70vh]" : "")}>
              ...
              {connectStep === 2 && selectedPlatform?.connectionType === 'manual_credentials' && (
                <>
                  ...
                  <p className="text-on-surface-variant font-medium leading-relaxed mb-8">
                    {selectedPlatform.name} không dùng đăng nhập thông thường. Bạn cần tạo một mật khẩu ứng dụng trong cài đặt {selectedPlatform.name} rồi nhập vào đây.
                  </p>

                  <div className="space-y-4 mb-8">
                    <div>
                      <label className="block text-sm font-bold text-on-surface mb-2">Tên tài khoản {selectedPlatform.name}</label>
              ...
              {connectStep === 2 && selectedPlatform?.connectionType === 'access_code' && (
                <>
                  ...
                  <h2 className="text-2xl font-bold text-on-surface mb-3">Kết nối {selectedPlatform.name}</h2>
                  <p className="text-on-surface-variant font-medium leading-relaxed mb-8">
                    Liên kết kênh hoặc nhóm {selectedPlatform.name} của bạn
                  </p>
